...
class MonodepthModel(object):
    ...
    def scale_pyramid(self, img, num_scales):
        ...
        for i in range(num_scales - 1):
            ...
            nw = w // ratio
            scaled_imgs.append(
                tf.image.resize(img, [nh, nw], method=tf.image.ResizeMethod.AREA)
            )
        return scaled_imgs
    ...
    def get_disparity_smoothness(self, disp, pyramid):
        ...
        weights_x = [
            tf.exp(-tf.reduce_mean(tf.abs(g), 3, keepdims=True))
    ...
    def maxpool(self, x, kernel_size):
        p = np.floor((kernel_size - 1) / 2).astype(np.int32)
        p_x = tf.pad(x, [[0, 0], [p, p], [p, p], [0, 0]])
        return layers.MaxPooling2D(pool_size=kernel_size, strides=2, padding="valid")(
            p_x
        )

    def resconv(self, x, num_layers, stride):
        do_proj = x.shape[3] != num_layers or stride == 2
        shortcut = []
    ...
    def deconv(self, x, num_out_layers, kernel_size, scale):
        ...
        conv = layers.Conv2DTranspose(
            num_out_layers, kernel_size, strides=scale, padding="same"
        )(p_x)
        return conv[:, 3:-1, 3:-1, :]
    ...
    def build_summaries(self):
        ...
        with tf.device("/cpu:0"):
            for i in range(4):
                ...
                tf.summary.scalar(
                    "lr_loss_" + str(i), self.lr_left_loss[i] + self.lr_right_loss[i]
                )
                tf.summary.image(
                    "disp_left_est_" + str(i), self.disp_left_est[i], max_outputs=4
                )
                tf.summary.image(
                    "disp_right_est_" + str(i), self.disp_right_est[i], max_outputs=4
                )

                if self.params.full_summary:
                    tf.summary.image(
                        "left_est_" + str(i), self.left_est[i], max_outputs=4
                    )
                    tf.summary.image(
                        "right_est_" + str(i), self.right_est[i], max_outputs=4
                    )
                    tf.summary.image(
                        "ssim_left_" + str(i), self.ssim_left[i], max_outputs=4
                    )
                    tf.summary.image(
                        "ssim_right_" + str(i), self.ssim_right[i], max_outputs=4
                    )
                    tf.summary.image(
                        "l1_left_" + str(i), self.l1_left[i], max_outputs=4
                    )
                    tf.summary.image(
                        "l1_right_" + str(i), self.l1_right[i], max_outputs=4
                    )
