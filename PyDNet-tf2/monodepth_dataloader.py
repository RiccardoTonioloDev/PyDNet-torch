...
class MonodepthDataloader(object):
    ...
    def __init__(self, data_path, filenames_file, params, dataset, mode):
        ...
        if mode == "test" and not self.params.do_stereo:
            left_image_path = tf.compat.v1.string_join([self.data_path, split_line[0]])
            left_image_o = self.read_image(left_image_path)
        else:
            left_image_path = tf.compat.v1.string_join([self.data_path, split_line[0]])
            right_image_path = tf.compat.v1.string_join([self.data_path, split_line[1]])
            left_image_o = self.read_image(left_image_path)
            right_image_o = self.read_image(right_image_path)

        if mode == "train":
            # randomly flip images
            do_flip = tf.compat.v1.random_uniform([], 0, 1)
            left_image = tf.cond(
                do_flip > 0.5,
            ...
            do_augment = tf.compat.v1.random_uniform([], 0, 1)
    ...
    def augment_image_pair(self, left_image, right_image):
        ...
        random_gamma = tf.compat.v1.random_uniform([], 0.8, 1.2)
        left_image_aug = left_image**random_gamma
        right_image_aug = right_image**random_gamma

        # randomly shift brightness
        random_brightness = tf.compat.v1.random_uniform([], 0.5, 2.0)
        left_image_aug = left_image_aug * random_brightness
        right_image_aug = right_image_aug * random_brightness

        # randomly shift color
        random_colors = tf.compat.v1.random_uniform([3], 0.8, 1.2)
    ...
    def read_image(self, image_path):
        ...
        file_extension = tf.compat.v1.substr(image_path, path_length - 3, 3)
        file_cond = tf.equal(file_extension, "png")

        image = tf.cond(
            file_cond,
            lambda: tf.image.decode_jpeg(tf.compat.v1.read_file(image_path)),
            lambda: tf.image.decode_png(tf.compat.v1.read_file(image_path)),
        )
