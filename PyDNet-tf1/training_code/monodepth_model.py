...
class MonodepthModel(object):
    ...
    def generate_image_left(self, img, disp):
        return tf.cast(
            bilinear_sampler_1d_h(tf.cast(img, tf.float32), tf.cast(-disp, tf.float32)),
            tf.float32,
        )
    ...
    def build_losses(self):
        with tf.variable_scope("losses", reuse=self.reuse_variables):
            ...
            # SSIM
            self.ssim_left = [
                self.SSIM(self.left_est[i], self.left_pyramid[i])
                for i in range(self.num_scale)
            ]
            self.ssim_loss_left = [tf.reduce_mean(s) for s in self.ssim_left]
            self.ssim_right = [
                self.SSIM(self.right_est[i], self.right_pyramid[i])
                for i in range(self.num_scale)
            ]
    ...
    def build_summaries(self):
        ...
        with tf.device("/cpu:0"):
            for i in range(self.num_scale):
                ...
                tf.summary.image(
                    "disp_left_est_" + str(i + 1),
                    self.disp_left_est[i],
                    max_outputs=1,
                    collections=self.model_collection,
                )
                tf.summary.image(
                    "disp_right_est_" + str(i + 1),
