...
def test_simple(params):
    ...
    input_image = input_image.resize(
        (args.input_width, args.input_height), Image.LANCZOS
    )
    ...
    disp_to_img = Image.fromarray((disp_pp.squeeze() * 255).astype(np.uint8))
    disp_to_img = disp_to_img.resize((original_width, original_height), Image.LANCZOS)
    disp_to_img.save(os.path.join(output_directory, "{}_disp.png".format(output_name)))
